...
#include "driver/uart.h" //for uart? shits dumb
#include "driver/spi_common.h"
#include "driver/spi_master.h" //between these two im not sure which i need, but both is probably a good start.

#include "time.h"
#include "sys/time.h"


#define SPP_TAG "ESP_SPP_PROJ" //PLACEHOLDER NAME
#define SPP_SERVER_NAME "SPP_SERVER"
#define EXAMPLE_DEVICE_NAME "ESP_BT_PSNEM"
#define SPP_SHOW_DATA 0
#define SPP_SHOW_SPEED 1
#define SPP_SHOW_MODE SPP_SHOW_DATA    /*Choose show mode: show data or speed*/

#define LED_RED 23 //for bt demo kinda
#define LED_GRN 22 //""


#define UART_TX2 17 //uart to msp430, this will be hard prob :(
#define UART_RX2 16 //""
...
#define ADC_CS 5 // output pin from adc, i think will be constant? dk atm
...
static long data_num = 0;  //dont rly care abt, maybe useful for debug..

static const esp_spp_sec_t sec_mask = ESP_SPP_SEC_AUTHENTICATE;
static const esp_spp_role_t role_slave = ESP_SPP_ROLE_SLAVE;


static void start_pins(void){ //setting up LEDS, UART, SPI  MAY OR MAY NOT SPLIT INTO 3 FUNCTIONS
    gpio_reset_pin(LED_GRN);
    gpio_reset_pin(LED_RED);
 //   gpio_set_direction(LED_GRN,GPIO_MODE_OUTPUT);
   // gpio_set_direction(LED_RED,GPIO_MODE_OUTPUT);
  //  ESP_LOGI(SPP_TAG,"LED PINS READY TO GO");
    ...
    spi_device_handle_t spi; //sure lol
    ...
    spi_bus_config_t Spibus = {
        ...
        .quadhd_io_num = -1,
        .quadwp_io_num = -1,
        .max_transfer_sz = 16 , // i think 4092 is the default, but might as well track it. Im only ever looking at 12 bits of 2s complement so idc about this too much
    };
    ...
    ESP_ERROR_CHECK( spi_bus_add_device(ESP_HOST,&SpiDev,&spi) );
...
void looper(void){ //porbably just for spi data, idc if uart goes here, as uart should only happen when bt sends data back
    int i = 1;
    int num = 1;
    uint8_t* data = (uint8_t*) malloc(30);
    char* test_buffer = "Testing now\n";
    while (1){ //while true loop
    int len = sprintf((char*)data, "Testing sprintf %d", num++);
    uart_write_bytes(uart_MSP,(const char*)data,len); //send same data as above to uart2, should be the msp430
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    //turns it to a string, then places up to (what should be) 126 characters of it to the received data buffer / location


    //NOW, i will use this to turn on an led for now, thought for the future this received data will need to be sent through uart to the msp430
    if (strncmp(received_data, "OFF", 3 ) == 0){//compared first two digits of received data with RED, if equal, returns 0
   //     gpio_set_level(LED_RED,1);//turns on red led
      //  gpio_set_level(LED_GRN,0);//turn off green led
        ESP_LOGI(SPP_TAG, "RED LED ON AND GREEN OFF"); //log info, not req
    ...
    else if(strncmp(received_data,"ON",2) == 0){
     //   gpio_set_level(LED_RED,0);//turns off red led
      //  gpio_set_level(LED_GRN,1);//turn on green led
        ESP_LOGI(SPP_TAG, "RED LED OFF AND GREEN ON"); //log info, not req
        esp_spp_write(param->data_ind.handle,param->data_ind.len, param->data_ind.data); //should just spew back input data as output? REMOVE/REPLACE LATER
        uart_write_bytes(uart_MSP,param->data_ind.data,param->data_ind.len); //send same data as above to uart2, should be the msp430

    }
    else{
        ESP_LOGI(SPP_TAG, "DID NOT RECOGNIZE"); //log info, not req

     //   gpio_set_level(LED_RED,1);//turns on red led
     //   gpio_set_level(LED_GRN,1);//turn on green led
       // vTaskDelay( 1000 / portTICK_PERIOD_MS); //dekay 1 sec
      //  gpio_set_level(LED_RED,0);//turns off red led
      //  gpio_set_level(LED_GRN,0);//turns off green led
        esp_spp_write(param->data_ind.handle,param->data_ind.len, param->data_ind.data); //should just spew back input data as output? REMOVE/REPLACE LATER
        uart_write_bytes(uart_MSP,param->data_ind.data,param->data_ind.len); //send same data as above to uart2, should be the msp430
    }
        break;
...
void esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param) // this just handles authentification
{
    char bda_str[18] = {0};
    switch (event) {
...
void app_main(void)
{
    ...
    looper(); //hopefully this starts regular functionality..
}
