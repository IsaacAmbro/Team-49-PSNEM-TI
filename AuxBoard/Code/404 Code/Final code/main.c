...
#define ADC_SCLK 18 // output pin to adc
#define ADC_DIN 23 //this is MOSI output pin to adc.
#define ADC_DOUT 19 //this is MISO input pin from adc  
...
static void start_pins(void){ //setting up UART, SPI 
    ...
    vTaskDelay(1000/portTICK_PERIOD_MS); //Wait to allow ADC setup time, subject to change
    ...
    spi_bus_config_t Spibus = {//setting up our pins
        .miso_io_num = ADC_DOUT,
        .mosi_io_num = ADC_DIN,
        .sclk_io_num = ADC_SCLK,
        .quadhd_io_num = -1,
        .quadwp_io_num = -1, //show unused basically
        //.max_transfer_sz = 4092 , // i think 4092 is the default, but might as well track it. Im only ever looking at ~12 bits of 2s complement so idc about this too much
    };
    ...
    spi_device_interface_config_t SpiDev = {//setup slave device
        .mode = 1,
        //.spics_io_num = ADC_CS,
        .clock_speed_hz = 1000 *1000, //ADC internal oscil at 1mhz try at 250 k ?
        .queue_size = 1, //kinda artbitrary for now..
        .flags = 0,
    };
    ESP_ERROR_CHECK( spi_bus_add_device(ESP_HOST,&SpiDev,&adc_spi) ); //initialize everything
    ESP_LOGI(SPP_TAG,"ADC PINS READY TO GO");
    gpio_set_direction(ADC_CS,GPIO_MODE_OUTPUT);
    uint32_t Low=0; //setting CS high or low
    gpio_set_level(ADC_CS,Low); // CS must be low for spi com, make sure its low
...
void looper(){ //for reading and writing spi
    ...
    //uint16_t config = 0b1100101111000101; //single shot, ainp is ain0, ainn is ain1, fsr 2, 3300 sps
    uint16_t config =0b0100010011001010; //cont
    config = (config>>8) | (config<<8); //for cont 

    //0b1100010111001011 is what I want to send. I have to swap the bytes bc spi and esp are odd
    //0b1100101111000101 WHAT I SEND!!!!

    //0b0 100 010 0 110 0 1 01 0 //continuous
    //0b0100010011001010 
    ...
    while (1){ 
        ...
        gpio_set_level(ADC_CS,Low);
        ret = spi_device_polling_transmit(adc_spi,&trans);
        //ret = spi_device_transmit(adc_spi,&trans);
        assert(ret==ESP_OK);
        gpio_set_level(ADC_CS,!Low);
        ADC_data = (ADC_data>>8) | (ADC_data<<8);
        ADC_data = ADC_data >> 4; // change to be not left justified
        if ( (ADC_data & 0b0000100000000000) == 2048 ){ //bitwise and to find if i have a negative number?
            ADC_data = ~ADC_data + 1; //NOT of adc DATA + 1 to to go regular  form with extra junk in front
            ADC_data = ADC_data^0b1111000000000000; // remove junk in front
            result =  -2.048 * ADC_data / (1<<11); //give me a voltage number but negative
        }
        else{
            result =  2.048 * ADC_data / (1<<11); //should just be positive
        }

        uint8_t *bytearray = (uint8_t*)&result; //just change this to result for ADC data
        int lengtarray = 4; //length in bytes. Annoying the function made me guess

        esp_spp_write(Gooboo,lengtarray,bytearray);
        printf("VALUE SENT: %f\n ", result );
        //i++;
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param) // CALLBACKS, ONLY EDITED DATA TRANSFER ONE
{
    ...
    if (strncmp(received_data, "1", 1 ) == 0){//
        ESP_LOGI(SPP_TAG, "RECEIVED \"LDO\""); //log info to upl uart
        esp_spp_write(param->data_ind.handle,param->data_ind.len, param->data_ind.data); //Sending what i sent to uart over b/t. Kept as backup, deleted for others
        uart_write_bytes(uart_MSP,param->data_ind.data,param->data_ind.len); //

    }
    else if(strncmp(received_data,"2",1) == 0){//looking for swtchreg
        ESP_LOGI(SPP_TAG, "RECEIVED \"BUCK\""); //log info
        uart_write_bytes(uart_MSP,param->data_ind.data,param->data_ind.len); //

    }

    else if(strncmp(received_data,"3",1) == 0){//looking for pwrsup
        ESP_LOGI(SPP_TAG, "RECEIVED \"PWRSUP\""); //log info
        uart_write_bytes(uart_MSP,param->data_ind.data,param->data_ind.len); //

    }
    else if(strncmp(received_data,"4",1) == 0){//looking for G
        ESP_LOGI(SPP_TAG, "RECEIVED \"G\""); //log info
        uart_write_bytes(uart_MSP,param->data_ind.data,param->data_ind.len); //
