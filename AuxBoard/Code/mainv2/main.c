...
#define SPP_TAG "ESP_SPP_PROJ" //used for logs I think
...
#define SPI_TAG "spi_protocol" //??? I dont know what exactly this does
...
static const spi_device_handle_t adc_spi; //name of our spi slave device
...
void looper(esp_spp_cb_param_t *param){ //for reading and writing spi
    esp_err_t ret;
    vTaskDelay(1000/portTICK_PERIOD_MS); //delay 1 second to ensure no issues

    uint16_t config = 0x48A; //Config reg is 16 bits 0-15
    //0b0000010010001010 ->0x48A 
    uint16_t conversion=0; //Convs reg is also 16 bits, this is my received data
    uint8_t step[4];
    //uint16_t step2[2];
    ...
    int i = 1; //these arte for looping
    int num = 1; 
    uint8_t* data = (uint8_t*) malloc(50); //setup data buffer

    while (1){ //forever loop
        int len = sprintf((char*)data, "From sprintf, loop no. %d", num++); //loads the buffer with output text that increments
        uart_write_bytes(uart_MSP,(const char*)data,len); //send same data as above to uart2 (which will be the msp430). Used to check that im connected properly and Im sending data from b/t as well
        ...
        recv.length =16; //still 16 bits total
        ...
        esp_spp_write(param->data_ind.handle,8, step[0]);  
        esp_spp_write(param->data_ind.handle,8,step[1] );  
        esp_spp_write(param->data_ind.handle,8,step[2] );  
        esp_spp_write(param->data_ind.handle,8, step[3]  );  
        //temporary
        vTaskDelay( 25 / portTICK_PERIOD_MS); //delay X/1000 secs between loop
    }
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param) // CALLBACKS, ONLY EDITED DATA TRANSFER ONE
{
    ...
    if (strncmp(received_data, "LDO", 3 ) == 0){//comparing for LDO all caps, first 3 digits of received data
        ...
        uart_write_bytes(uart_MSP,param->data_ind.data,3); //send same data as above to uart2, should be the msp430
        ...
        uart_write_bytes(uart_MSP,param->data_ind.data,8); //send same data as above to uart2, should be the msp430
    }

    else if(strncmp(received_data,"PWRSUP",6) == 0){//looking for pwrsup
        ESP_LOGI(SPP_TAG, "RECEIVED \"PWRSUP\""); //log info
        esp_spp_write(param->data_ind.handle,param->data_ind.len, param->data_ind.data); //should just spew back input data as output
        uart_write_bytes(uart_MSP,param->data_ind.data,6); //send same data as above to uart2, should be the msp430.should only be 6, dont want to send extra if there is any
    }
    else{
...
void app_main(void) //half I dont touch, dont want to break
{
    ...
    looper(esp_spp_cb); //start our loop to grab ADC data, if this works i would be suprised
}
